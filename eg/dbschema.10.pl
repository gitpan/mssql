#---------------------------------------------------------------------
# $Header: /Perl/MSSQL/eg/dbschema.10.pl 2     99-01-30 16:41 Sommar $
# Comments below are from Michael Peppler, you will need to change this
# to use it with MSSQL Server 6.x.
#
# $History: dbschema.10.pl $
# 
# *****************  Version 2  *****************
# User: Sommar       Date: 99-01-30   Time: 16:41
# Updated in $/Perl/MSSQL/eg
# Changed some defaults and some obvious errors.
#
# *****************  Version 1  *****************
...
&Getopts('u:d:t:o:s:v:p:');

$opt_u = $ENV{'USER'} unless $opt_u;
...
$dbproc = MSSQL::DBlib->dblogin("$opt_u", $opt_p, $opt_s);
$dbproc->dbuse($opt_d);
$dbproc->{NullIsUndef} = 0;
...
foreach (@tables)    # For each line in the list
{
    ...
    $dbproc->dbcmd( "       Prec = c.prec, \n");
    $dbproc->dbcmd( "       Scale = c.scale, \n");
    ...
    $dbproc->dbcmd( "       Ident = convert(bit, (c.status & 0x80)) ");
