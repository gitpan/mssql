# $Header: /Perl/MSSQL/docs/mssql-dblib.pod 9     03-01-01 16:01 Sommar $
#
# $History: mssql-dblib.pod $
# 
# *****************  Version 9  *****************
# User: Sommar       Date: 03-01-01   Time: 16:01
# Updated in $/Perl/MSSQL/docs
# Updated for MSSQL 1.009.
#
# *****************  Version 8  *****************
# User: Sommar       Date: 01-10-21   Time: 14:57
# Updated in $/Perl/MSSQL/docs
# Added dbgetpacket, dbsetmaxprocs, dbgetmaxprocs and a note on
# dbsetlpacket.
#
# *****************  Version 7  *****************
# User: Sommar       Date: 01-05-05   Time: 18:30
# Updated in $/Perl/MSSQL/docs
# MSSQL:DBlib 1.008: Added more text/image functions for a new section in
# the manual. Made the text on dbsetopt clearer.Fixed some other minor
# errors. Use C<> for $vars when needed.
#
# *****************  Version 6  *****************
# User: Sommar       Date: 00-10-26   Time: 23:24
# Updated in $/Perl/MSSQL/docs
# Fixed incorrect documentation.
#
# *****************  Version 5  *****************
# User: Sommar       Date: 00-09-14   Time: 22:48
# Updated in $/Perl/MSSQL/docs
...
B<CAVEAT> Use of DB-Library is deprecated by Microsoft, and you do not
...
not use MSSQL::DBlib for new development.

As far as the restrictions apply directly to MSSQL::DBlib, I discuss them
below. There is also a summary on L<mssql-sqllib>. For a full discussion,
...
C<$server> may refer to a named instance, for instance MYMACHINE\SECOND.
B<Note:> Named instances were introduced in SQL2000, and according
...
I<Client Network Utility> to set up an alias for the named instance.
...
In this case SQL Server will ignore C<$user> and C<$pwd>.
...
Make C<$database> the current database.

=head3 dbcmd

   $status = $dbh->dbcmd($sql_cmd)

Appends the string C<$sql_cmd> to the current command buffer of this connection.
...
can be meaningful calling L</dbdataready> before calls to L</dbresults> and
...
until it returns NO_MORE_RESULTS, or cancel the batch with L</dbcancel>.
...
Get the next row from the current result set. If the second parameter C<$wantref>
has a false value, the return value is a list of scalar values. If the first
parameter C<$use_hash> has a true value, the names of the columns are included
in the list, so that you can receive the data into a hash.

When C<$wantref> has a true value, a reference to an array or a hash is
returned, depending on C<$use_hash>.

The return status from the DB-Library function I<dbnextrow> is available
in C<$dbh-E<gt>{DBstatus}>.
...
is available in C<$dataref>. When a fetch fails (see possible status values below),
C<$dataref> will be C<undef>.

C<$dataref> is a reference to an array (when C<$use_hash> has a false value or is
omitted) or a hash (when C<$use_hash> has a true value). When returning an
...
B<bigint, float, real, numeric, decimal, money, smallmoney:>  As float values
(default) or strings, depending on the handle attribute C<dbKeepNumeric>.

B<char, varchar, nchar, nvarchar, sql_variant:> As strings. Values longer than
...
always returned as char values, no matter the actual base type.
...
Clears C<$n> rows from the row buffer. Row buffering only applies if you have set
...
Returns the column type of column C<$colid> in the current result set. Constants
...
Returns the maximum length (in bytes) of column C<$colid> in the current result
set.
...
Returns the name of column C<$colid> in the current result set.

=head3 dbstrcpy

   $string = $dbh->dbstrcpy [($start [, $numbytes])]

Returns the contents of the command buffer. C<$start> is the position to
start copying from. Default is 0, the first character in the buffer.
When negative, C<$numbytes> asks for the entire buffer to be returned.
When >= 0, at most C<$numbytes> characters are returned. Default is -1,
return everything.
...
Sets option C<$opt> with optional character parameter C<$char_val>. C<$opt> is one
...
There are two kinds of options: SQL Server options (that have corresponding SET
commands) and DB-Library options (that have no corresponding T-SQL commands at
all.)

For the SQL Server options L</dbsetopt> does not itself send the SET command to
SQL Server, you must use L</dbsqlexec> and L</dbresults> just like after sending
a command before the SET command to take effect, as shown in the example above.

But for DB-Library options, you B<must not> use the sequence with L</dbsqlexec>
and L</dbresults>, because since no command buffer has been initiated,
L</dbresults> will never return NO_MORE_RESULTS, but appears to return SUCCEED
and then FAIL.
...
the first two as DB-Library options.)
...
Unsets the option C<$opt>, which might be on by default, or previously have been set
with L</dbsetopt>.

See L</dbsetopt> for when and when not calling L</dbsqlexec> and L</dbresults>
after calling L</dbsetopt>.

=head3 dbisopt

   $ret = $dbh->dbisopt($opt [, $char_val])

Returns a true value if the option C<$opt> is set.

=head3 dbgetpacket

  $size = $dbh->dbgetpacket()

Returns the current packet size of the connection. You can set the
packet size prior to opening a connection with L</dbsetlpacket>.
...
C<$offtype> is a constant telling what you want offsets to. These constants
are defined in MSSQL::DBlib::Const::Offset. C<$startfrom> is where to start
searching in the command buffer. Typically you would set C<$startfrom> to
the last returned C<$offset> + 1 for the next search.
...
C<$err_handler> should be a reference to a Perl C<sub>, or a string containing
...
blessed. When C<$dbproc> is blessed, it is the handle you used for calling
...
may have occurred. When there is no such error, C<$oserr> is DBNOERR, which is
...
C<$msg_handler> should be a reference to a Perl C<sub>, or a string containing
the name of a Perl C<sub>. In the latter case, specify the name with the
...
B<$dbproc>: This is a reference to a hash. The reference may or may not be
blessed. When C<$dbproc> is blessed, it is the handle you used to call the
...
C<$dbproc>. Only when C<$dbproc> is blessed you can call other MSSQL::DBlib
...
L</dbopen>. C<$onoff> should be any of the strings "ON" or "OFF". See Books
...
If you do not set the packet size, the default is determined by the
...
usually gives performance less than optimal.
...
to the server with L</dblogin> or L</dbopen>. C<$seconds> can also be any of the
...
Sets the maximum number of connections you can have open simultaneously
in your script. The default is 25.

=head3 dbgetmaxprocs

    $maxprocs = dbgetmaxprocs()

Returns the maximum number of connections you can have open simultaneously
in your script. You can set this value with L</dbsetmaxprocs>.
...
B<NOTE:> in the consequence with the above note, do not call C<dbexit> at
all from code that is being called by ActiveState's PerlScript or something
else which keeps the DLL for MSSQL::DBlib loaded.

=head3 reformat_uniqueid

    $GUID = MSSQL::DBlib::reformat_uniqueid($hexstring)

Converts C<$hexstring> which supposedly have been returned by L</dbnextrow2> as
the value for a column of the type uniqueidentifier to the regular presentation
form for a GUID. If C<$hexstring> is already on this format, C<$hexstring> is
returned unchanged. If C<$hexstring> is not a 32 character long hexstring (or 34
...
B<Note:> Assume that you are restricted to use 6.5 datatypes. If this is not
sufficient use the BCP that comes with SQL7 and SQL2000, the BULK INSERT
command or the BCP API of ODBC or OLE DB.
...
C<BCP_SETL> with C<$state> = 1 I<prior to> creating that connection
...
Initializes a bulk-copy operation. C<$table> is the table you are bulking into
or out from. C<$datafile> is a file that the data is read from or written to.
Specify C<undef> if you are sending data from variables with L</bcp_sendrow>.
C<$errfile> is a file for BCP to write error messages to. C<$direction> must
...
Sends the data in C<@list> to the server. C<@list> is assumed to contain one
...
file and SQL Server. The available values for C<$field> are listed below. Note
that for all parameters, specifying 0 for C<$value> will give you the default
value.
...
Initializes an RPC call to the stored procedure C<$SP>. Available options are
...
datatype or a datetime datatype for C<$type>. In the former case, conversion from
...
indicate that you can set C<$maxlen> to -1 in all cases, except when specifying
...
non-NULL fixed-length datatypes. Specify 0 for C<$datalen> to send NULL for
all datatypes. If you want to send an empty string, you need to actually send
one blank. C<$datalen> cannot exceed 255.

B<$value> is the value for the parameter. When C<$datalen> is 0, C<$value> is
ignored. When sending in a binary type, C<$value> may, but does not need to,
...
C<$wait> determines if you want to wait for SQL Server to return a reply. The
default is 1, to wait.

When you specify a false value for C<$wait>, you must later call L</dbsqlok>
...
If the second parameter C<$wantref> has a false value, the return value is a
list of scalar values. If the first parameter C<$use_hash> has a true value,
...
L</dbretdata2> returns a reference, which is a reference to an array if C<$use_hash>
...
=head2 Text/image functions

The functions for inserting and updating text/image columns in MSSQL::DBlib
have a different interface from B<dbwritetext> and B<dbupdatetext> in
DB-Library.

There is a simple example of using L</dbwritetext> in F<eg\wtext.pl>. For
example of all functions, please see the test script F<dblib\t\3_text.t>.
...
You can use these calls to remove all limits:
...
B<text in row> (available in SQL2000 an on).

General caveat: about the only time I play with text/image columns is
when I work with the MSSQL::DBlib.

=head3 dbreadtext

   $bytes = dbh->dbreadtext($buf, $len)

You can always use L</dbnextrow2> to retrieve a text/image value in its
...
end you use L</dbreadtext> in place of L</dbnextrow2>.

To use to L</dbreadtext> you must submit a query with the one single
column, the text/image column you want to retrieve.

C<$buf> will receive with chunk of data. C<$len> is how much data you want back
at a maximum.

The return value C<$bytes> can have any of these values:
...
Note that the limits set by DBTEXTSIZE and DBTEXTLIMIT apply to L</dbreadtext>
as well.
...
C<$colname> is the name of the text column, including table name.

C<$dbh_2> is the handle of another server connection, and which should have an
active query against the table of the text column. C<$colname> should be
part of that result set.

C<$colnum> is the number of the text column I<in that query> (thus, not in
the table).

C<$text> is the text to be inserted.

C<$log> only has importance if the recovery model of the database is
BULK_LOGGED (in SQL7 and earlier this corresponds to the database
option I<select into/bulk copy> set to true). In this mode C<$log>
controls whether the operation is actually logged or not. The default is
0, not to log. In other recovery models, L</dbwritetext> is always fully
...
See also F<eg\wtext.pl> for a complete script.

=head3 dbpreptext

   $status = $dbh->dbpreptext($colname, $dbh_2, $colnum, $size, [, $log])

Prepares the insertion/update of text for later calls with L</dbmoretext>.
C<$colname>, C<$dbh_2>, C<$colnum> and C<$log> works in the same way as with
L</dbwritetext>. C<$size> is the size in bytes of the data you intend to
insert with L</dbmoretext>.
...
Note that you twice must call L</dbsqlok> followed by L</dbresults> in this
sequence. First once after calling L</dbpreptext> and then once you have
finished calling L</dbmoretext>.

=head3 dbmoretext

   $status = dbh->dbmoretext($len | undef, $buf);

Adds a chunk of data to a text or image column. You must first have
called L<dbpreptext> to initiate the operation. C<$buf> is the data you
send in, C<$len> is the length of the data you write. You can pass C<undef>
instead of C<$len>, in which case L</dbmoretext> will use take the length
from C<$buf>. (The interface may appear illogical, but I've preserved the
parameter order from Sybperl.)
...
Updates a portion of a text/image column. For the parameters C<$colname>,
C<$dbh_2>, C<$colnum>, C<$text> and C<$log> please see the description for L</dbwritetext.>

C<$insert_offset> specifies where in the column C<$text> is to be inserted and
...
C<$text> will be appended to the existing value.

C<$delete_length> specifies how many bytes that are to be deleted from the
existing value in the column. The deletion starts at C<$insert_offset>. -1
menas that all bytes from C<$insert_offset> to the end are deleted. The
default is 0, which means that no bytes are deleted.

=head3 dbprepupdatetext

   $status = $dbh->dbprepupdatetext($colname, $dbh_2, $colnum, $size,
                                   [$insert_offset, [$delete_length, [, $log]]])

L</dbprepupdatetext> corresponds to L</dbupdatetext> as L</dbpreptext>
corresponds to L</dbwritetext>. That is, you are supposed to specify how
much data you want to insert with C<$size> and then call L</dbmoretext> to
insert the data.
...
B<dbupdatetext> with the flag UT_MORETEXT does not work at all. b) MSSQL::DBlib
plays tricks in L</dbnextrow2> that breaks the scheme. c) I have simply not
understood how the calling sequence should be.

In the vain hope that the right answer is c), I have included and documented
L</dbprepupdatetext>, but you are warned.
...
Deletes C<$delete_length> bytes from the text/image column, starting at
C<$insert_offset> which must be >= 0. No new bytes are inserted to the column.

For C<$colname>, C<$dbh_2>, C<$colnum> and C<$log>, see L</dbwritetext>.
...
L</dbcopytext> copies text from one text column to another.

C<$colnamedest> is the name of the table and column to copy to. C<$colnamesrc> is
the table and column to copy from.

C<$dbh_dest> is the handle of another server connection, and which should have an
active query against the destination column. C<$colnamedest> should be
part of that result set.

C<$colnumdest> is the number of C<$colnamedest> I<in that query> (thus, not in
the table).

C<$dbh_src> and C<$colnumsrc> are the equivalent for the source column.

C<$insert_offset>, C<$delete_length> and C<$log> works as for L</dbupdatetext>.
...
=head1 Notes on threading and PerlScript

MSSQL::DBlib is B<not> thread-safe with scripts that uses C<use threads>.

However, you can use it with ActiveState's PerlScripts in things like
...
have parallel invocations. Here are some things to keep in mind:
...
on global level will affect all threads. They are: C<dbsettime>,
C<dbsetlogintime>, C<dbsetmaxprocs>, C<DBGETTIME>, C<dbgetmaxprocs>
and C<dbexit>. Thus some care is needed when calling these functions.

=item *

Never call C<dbexit> in the context of PerlScript. You are closing down
DB-Library, and you could wreck another thread's handle.

=item *

When you set up an error or message handler with C<dberrhandle> or
C<dbmghandle>, the handler will be local to the thread. (This is
different from how you work with DB-Library in C.)

=item *

If you are expecting many parallel executions, you may need to change
the limit of current DBPROCESSes with C<dbsetmaxprocs>. The default
value is 25.

=back

Finally a caveat: I have actually only tested with DTS. If you run into
problems with ISAPI, there is a workaround. Set this registry key to 0:
...
I<DBSETLCHARSET>, I<bcp_getl>, the I<dbmny*> routines,
