$Header: /Perl/MSSQL/docs/mssql-sqllib.pod 7     01-05-05 18:31 Sommar $
...
what it supports of the features added in SQL 7 and later. See the section
L</SQL 7 and beyond> at the end of this document.
...
B<$server> may refer to a named instance, for instance MYMACHINE\SECOND, B<but:>
Microsoft does not officially support named instances with DB-Library. Nevertheless
Microsoft have added some code to DB-Library so that it can understand named instances.
It appears that it works with SQL Server authentication but not Windows NT
authentication.
...
by the parameters $rowstyle and C<$resultstyle>. $rowstyle determines how individual
rows appear, $resultstyle how the rows are structured. Below I say that the
...
In the call syntax above, $rowstyle is shown as coming before C<$resultstyle>.
However, L</sql> actually permits you to put $rowstyle after C<$resultstyle>.
This is also true when $resultstyle is KEYED and C<\&callback>. However, $rowstyle
must come before C<\@keys>.
...
$server_cs is the character set of the server (SQL7 and earlier) or the
...
=head1 SQL 7 and beyond

With MS SQL 7, Microsoft abadoned further development of DB-Library, on
which MSSQL::Sqllib is based, including adaptions to new features in SQL Server.

The following restrictions apply for features that were added to SQL Server
in SQL7 and later versions when using MSSQL::Sqllib and MSSQL::DBlib.
As a general rule: you can always use new or enhanced datatypes, as long
as you can pass them as part of a command string. But if you pass them
as a parameter with L</sql_sp> or retrieve them in a result set in
L</sql>, L</sql_one> or L</sql_sp> you are likely to encounter restrictions.
