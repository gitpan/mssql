# $Header: /Perl/MSSQL/docs/mssql-sqllib.pod 8     03-01-01 16:01 Sommar $
#
...
#
# *****************  Version 7  *****************
# User: Sommar       Date: 01-05-05   Time: 18:31
# Updated in $/Perl/MSSQL/docs
# MSSQL::Sqllib 1.008: $rowstyle and $resultstyle can now be swapped.
# sql_one now accepts extra result sets as long as they are empty.
# Specified the result from sql() when there is no result set.
#
# *****************  Version 6  *****************
# User: Sommar       Date: 00-09-14   Time: 22:49
# Updated in $/Perl/MSSQL/docs
# Updates for SQL 2000 and MSSQL::Sqlib .1007.
#
# *****************  Version 5  *****************
...
Since MSSQL::Sqllib is based on DB-Library, and Microsoft is no longer
developing DB-Library, this means that with MSSQL::Sqllib you do not have
access to all features in SQL7 and SQL2000. See the section L</Restrictions
with new datatypes> at the end of this document. Thus, you should not
...
is that you I<cannot> use it with C<use threads> in a Perl script, but you can
use it in an environment for a thing like ActiveState's PerlScript. Please
see further L<mssql-dblib/Notes on threading and PerlScript>.
...
Imports all constants, as well as C<$SQLSEP>, the only control variable
...
were you to accidently omit C<$database>.

To use B<Windows NT authentication> (known as "integrated security" MS SQL 6.5),
call the MSSQL::DBlib routine L<mssql-dblib/DBSETLSECURE> before you call
L</sql_init>.
...
B<$server> may refer to a named instance, for instance MYMACHINE\SECOND.
...
attributes, particularly L</errInfo>. In the syntax descriptions below, the
handles are shown as mandatory.

B<NOTE:> If you use MSSQL::Sqllib from a thing like ActiveState's PerlScript
you must always use the handle.
...
B<$X> is the handle that you got when you called L</sql_init>.

B<$sql> is a string containing one
...
For details on how the various data types in SQL Server are returned, see
...
by the parameters C<$rowstyle> and C<$resultstyle>, as detailed below. Below I say
...
In the call syntax above, C<$rowstyle> is shown as coming before C<$resultstyle>.
However, L</sql> actually permits you to put C<$rowstyle> after C<$resultstyle>.
This is also true when C<$resultstyle> is KEYED or C<\C<&callback>>. However,
C<$rowstyle> must come before C<\@keys>.
...
module attribute C<$MSSQL::Sqllib::SQLSEP>. Initially C<$SQLSEP> has the value
C<"\022">, a control character. You can set it to a string of any length that
fit your needs. Note, however, that the value of C<$SQLSEP> I<must not> appear
in the actual data.
...
arrays or scalars depending on C<$rowstyle>. The order of the array is the same
...
structure there is a hash, array or scalar, depending on C<$rowstyle>, with the
non-key columns of the result set.

Which S<column(s)> that supply the key values, is determined by the parameter
B<@keys>, which is mandatory for C<KEYED>. For row style C<HASH>, C<@keys> should
hold the name of the key S<column(s)> of the result set. For row style C<LIST>
and C<SCALAR>, C<@keys> should hold the column number of the S<key(s)>. Column numbers
start on 1.
...
The keys you provide in C<@keys> are supposed to be the unique keys of your result
set, and appear in all rows of the result S<set(s)>. If a key listed in C<@keys>
...
does not compile, and you have changed L</errInfo> so that L</sql> does not
abort on errors - you will get C<undef> in return if you specify a scalar return
value. Else you will get an empty list/hash.

B<Callbacks>

Instead of a result style, you can provide a callback. L</sql> calls your callback
in this way, depending on C<$rowstyle>:
...
B<$X> is the handle you got from L</sql_init>.

L</sql_sp> calls the stored procedure B<$sp> and retrieves the return status of
...
It is undefined whether output parameters of the datatypes I<binary> and I<char>
...
flavours, and we also want the return value. In T-SQL the call would be:
...
L<checkRetStat> attribute, as else L<sql_sp> would abort if the return value
was non-zero.
...
L</sql_string> returns C<$string> surrounded by single quotes, and any single quote
in C<$string> is doubled. If C<$string> is C<undef>, the string NULL (unquoted) is
returned. Example:
...
For C<$client_cs> you can also specify "OEM" or "ANSI" to use the OEM or ANSI
...
The default for C<$client_cs> is the local OEM character set. The default for
C<$server_cs> is the character set of the server (SQL7 and earlier) or the
default collation for the server (SQL2000 and later). (Use I<sp_helpsort>
...
@@version, and also in I<xp_msver>, for instance 8.00.194. (The release
version of SQL 2000.)
...
that is to be run by B<isql> later.
...
set C<dieFlag> for an error if C<neverStopOn>C<{$msgno}> is set. For DB-Library
errors, the error numbers are negated. Thus if C<neverStopOn>C<{-$dberr}> is set,
...
set C<dieFlag> for an error if C<alwaysStopOn>C<{$msgno}> is set. For DB-Library
errors, the error numbers are negated. Thus if C<alwaysStopOn>C<{-$dberr}> is set,
...
This is a hash with error numbers as keys. If C<neverPrint>C<{$msgno}> has a
true value, L</sql_message_handler> will print no part of the message
regardless of C<printMsg>, C<printText> and C<printLines>.

Likewise L</sql_error_handler> will not print a messages if C<neverPrint>C<{-$dberr}>
...
This is a hash with error numbers as keys. If C<alwaysPrint>C<{$errno}> has
...
the messages 3606, 3607 are flagged. 3606 is I<Arithmetic overflow
occurred>, 3607 is I<Division by zero occurred> and 3622 is
I<A domain error occurred>. These are raised with severity 0 if you have
...
=head1 Restrictions with new datatypes

With MS SQL Server 7, Microsoft abadoned further development of DB-Library, on
which MSSQL::Sqllib is based, including adaptions to new features in SQL Server.

The area where is most likely to give cause for concern is new and enhanced
...
procedure, or get them back in a result set. This is a list over known issues.
...
the value NULL, it will appear as 0 in DB-Library, not as C<undef>. The same
applies if you attempt to pass C<undef> for a bit parameter with L</sql_sp>.
...
parameters with L</sql_sp>.

SQL 7 introduced the empty string as a possible varchar value. These will
...
char values to DB-Library clients, and all data you enter through L</sql_sp>
...
Michael Peppler included the C<&sql> routine in his Sybperl, a contribution he
credits to Gisele Aas. This was the feather from which the Sqllib bird was
born.

=head1 AUTHOR

Erland Sommarskog  E<lt>F<sommarC<@algonet>.se>E<gt>.
